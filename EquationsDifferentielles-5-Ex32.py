...
f= lambda y,x:  x**2-y # l'équatino différentielle sous la forme y'=f(y,x)
sol=lambda x,K: K * np.exp(-x) + x**2-2*x+2 # expression des solutions


# trace les axes
xx,XX= -4.5, 4.5
yy,YY=plt.ylim(-3,5) # limite l'axe des ordonnées
...
plt.title(r"$ y'+y=x^2$",fontsize=20)
plt.xlabel(r"$ y(x) = K\cdot \exp(-x) + x^2-2x+2$",fontsize=15)
plt.legend()
plt.grid()
