...
f= lambda y,t: ((t-1)*y+np.exp(t))/t
sol=lambda t,k: k*np.exp(t)/t+np.exp(t)
...
ec=.1
LineStyle=['-', '--'] # style de ligne
CI=[(-1,1),(-2,-1)] # conditions initiales
couleurs=[(.4,.75,.5), (.3,.2,.4)] # couleur des courbes définies avec les valeurs (red,green, blue)
for (x0,y0), col,LS in zip(CI,couleurs,LineStyle):
    T1=np.linspace(x0,xx)
    T2=np.linspace(x0,-ec)
    Z1=spig.odeint(f,y0,T1)
...
plt.title(r"$ ty'-(t-1) y= \exp(t) $",fontsize=20)
plt.xlabel(r"$y(t) = k\cdot\frac{\exp(t)}{t} + \exp(t)$")
plt.legend()
plt.grid()
