...
f= lambda y,x:  (x*np.exp(-x/2)-y)/2 # l'équation différentielle sous la forme y'=f(y,x)
sol=lambda x,K: (K + x**2/4) * np.exp(-x/2)   # expression des solutions
...
plt.title(r"$ 2y'+y=x\cdot \exp\left(-\frac{x}{2}\right)$",fontsize=20)
plt.xlabel(r"$ y(x) = \left(K + \frac{x^2}{4} \right)   \cdot \exp\left(-\frac {x}{2}\right)$",fontsize=15)
plt.legend()
plt.grid()
