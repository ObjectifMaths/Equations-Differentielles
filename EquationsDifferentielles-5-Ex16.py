...
# y=Y[0] et z=Y[1]
f=lambda Y,x: ( Y[1] , Y[0]/2+Y[1]/2+3/2*np.exp(x) )



# trace les axes
xx,XX= -3,3
yy,YY=plt.ylim(-2,6) # limite l'axe des ordonnées
plt.plot([0,0],[yy,YY],'k-', [xx,XX],[0,0],'k-')
plt.axes().set_aspect('equal')
 
CI=[(0,1,0),(1,0,-1)] # conditions initiales
...
plt.title(r"$ 2y'' -y'-y =3\exp(x)$",fontsize=20)
plt.legend()
plt.grid()
